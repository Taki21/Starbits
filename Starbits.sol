...
contract Starbits is Context, IERC20, Ownable {
    ...
    function stakedBalance() public view returns (uint256) {
        return _stakedBalance[msg.sender];
    }
    ...
    function stake(uint256 amount) external updateReward(msg.sender) {
        require(_stakedBalance[msg.sender] + amount <= _balances[msg.sender]);
        _totalStaked += amount;
        _stakedBalance[msg.sender] += amount;
    }
    
    function unstake(uint256 amount) external updateReward(msg.sender) {
        require(_stakedBalance[msg.sender] - amount >= 0);
        _totalStaked -= amount;
        _stakedBalance[msg.sender] -= amount;
    }
    ...
    function unstakeAndGiveReward() private {
        uint256 reward = rewards[msg.sender];
        rewards[msg.sender] = 0;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        if(_balances[msg.sender] - amount < _stakedBalance[msg.sender]) { 
            unstakeAndGiveReward();
        }
